# FAIRWAY 프론트엔드 — 에이전트 지침

교통사고 과실비율 분석 서비스. 채팅형 단일 페이지 앱. 금융 AI 챌린지 해커톤 출품작.
**심사 9/7~9/11. 프로덕션 배포는 9/6에 고정하고 그 뒤엔 핫픽스만.** 프론트는 1인.

서비스 이름은 **FAIRWAY**다 (9/6 확정. 그전 가칭은 "카-디펜더"). 그래도 이름을 코드에 박지 말고
`src/config.ts`의 `APP_NAME`을 쓴다 — 로고·머리글·메일 제목이 전부 그 한 줄에서 나온다.
...
인정기준 도표 번호 · 접수번호 필수 여부 · 영상 제한 수치 · 메일 실제 발송 여부.
(서비스 이름은 9/6에 FAIRWAY로 확정됐다 — 그래도 값은 `APP_NAME`에서 가져온다.)
